...
3. **Locking migrations require explicit project-owner approval.** Anything that takes a write lock on a large or business-critical table.
...
7. **Webhook URL env var assertion before deploy.** `STRIPE_WEBHOOK_URL` must match production hostname. See `agent/memory/learnings/mistakes.md` 2026-04-28.
