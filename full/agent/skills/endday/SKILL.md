...
### 1. Append to `agent/memory/daily/YYYY-MM-DD.md`
...
### 2. Update `agent/memory/STATE.md` if priorities shifted
...
- `agent/agents/<name>/STATUS.md` — append a dated entry about what happened
- `agent/agents/<name>/MEMORY.md` — only if a non-obvious lesson emerged
- `agent/agents/<name>/PROJECT_MAP.md` — only if structural changes (new files/dirs)

Per `agent/rules/agent-quality.md` memory-loop scaling — match update depth to commit class.

### 4. If the user corrected you today

Append entry to `agent/memory/learnings/mistakes.md` MISTAKES section with the format:
...
```
Saved to memory:
- agent/memory/daily/YYYY-MM-DD.md (N actions, N decisions)
- STATE.md (updated priorities) [if applicable]
- agent/agents/<name>/STATUS.md [if applicable]
- agent/memory/learnings/mistakes.md (N new mistakes recorded) [if applicable]
...
- Writing a 2-page novel in agent/memory/daily/ — bullets, not paragraphs
...
- Skipping the user-correction → agent/memory/learnings/mistakes.md step (highest-value transcript output)
