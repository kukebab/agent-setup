---
name: call-debrief
description: Extract decisions, action items, participants, and build implementation plan from call transcript OR audio recording. Auto-detects project context. Saves structured markdown debrief to agent/memory/outputs/call-debriefs/. Updates agent/memory/projects/ and agent/memory/daily/. Use when user pastes a transcript, drops audio, or says "call debrief", "call notes", "what was decided", "transcript".
---
...
- User says: "call debrief", "call notes", "what was decided", "transcript" (or the equivalent phrase in the user's preferred language — see `agent/rules/language.md` if defined)
...
- **Client/project context** — does this match an existing `agent/memory/projects/<name>.md`?
...
File: `agent/memory/outputs/call-debriefs/YYYY-MM-DD_<topic-slug>.md`
...
[Optional: full transcript here, or link to agent/memory/raw/ if very long]
```

### 4. Update related state files

- **If client call** — update `agent/memory/projects/<name>.md`:
  - "Recent events" section: add dated entry
  - "Open asks" section: add new requests
  - "Current state as of" date: bump to today
- **Always** — update `agent/memory/daily/YYYY-MM-DD.md`:
...
```
Debrief saved: agent/memory/outputs/call-debriefs/YYYY-MM-DD_<slug>.md
...
Updated:
- agent/memory/projects/<name>.md (if client call)
...
- Skipping client file update — the debrief disappears in agent/memory/outputs/ otherwise
