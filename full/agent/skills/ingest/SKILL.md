---
name: ingest
description: Reflexive Karpathy LLM-wiki Ingest. Save external source (URL, gist, YouTube, repo, article, PDF, file path, or pasted text) to agent/memory/raw/, extract topics, update agent/memory/wiki/, and log in agent/memory/daily/. Use when user shares a URL/video/article/repo/gist/transcript, drops a file path, pastes a large block of text, or says "ingest this", "save this", "remember this article", "add to memory", "ingest <url>". Do NOT use for one-off lookups where the user doesn't want to keep the content.
---

# /ingest — Karpathy LLM Wiki Ingest

The **Ingest** operation from Karpathy's pattern. New source → `agent/memory/raw/` → update `agent/memory/wiki/` → log `agent/memory/daily/`.
...
```
File: agent/memory/raw/YYYY-MM-DD_<slug>.md
```
...
**Never edit `agent/memory/raw/` files later.** Frozen snapshot.
...
### 3. Update `agent/memory/wiki/`

For each topic touched:

- **New topic** → create `agent/memory/wiki/<topic>.md` with:
...
Update `agent/memory/wiki/INDEX.md` with new entries.

### 4. Append to `agent/memory/daily/YYYY-MM-DD.md`

```markdown
## Ingest

- **[Source title]** ([type: article/video/repo/etc.])
  - Saved to `agent/memory/raw/YYYY-MM-DD_slug.md`
...
```
Ingested: [title]
- raw: agent/memory/raw/YYYY-MM-DD_slug.md
- wiki updates: [list]
- daily logged
...
- Saving only to agent/memory/raw/ without updating wiki — half-job, wiki doesn't compound
- Editing agent/memory/raw/ later — destroys verifiability
- Creating shallow wiki pages — if a topic only has 2 sentences, fold it into an existing page
- Re-ingesting a source already in agent/memory/raw/ (check first)
