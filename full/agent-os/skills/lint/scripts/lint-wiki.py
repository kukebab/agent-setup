...
Usage:
  python3 .claude/skills/lint/scripts/lint-wiki.py

Optional env:
  WIKI_DIR — override wiki directory (default: ./memory/wiki)
"""
import os
import re
import sys
...
if not orphans:
    ...
    for name, updated in orphans:
        print(f"- `wiki/{name}.md` (updated: {updated})")
print()
...
if not broken:
    ...
    for filename, lineno, ref in broken:
        print(f"- `wiki/{filename}` line {lineno} → `[[{ref}]]`")
print()
...
if not no_summary:
    ...
    for name, first in no_summary:
        print(f"- `wiki/{name}.md` — starts with: `{first}...`")
print()
...
if not tiny:
    ...
    for name, size in tiny:
        print(f"- `wiki/{name}.md` ({size} bytes)")
print()
