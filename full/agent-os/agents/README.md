...
For small projects, one main agent (no `agents/` folder) is fine.

## Anatomy of an agent

Each agent is defined by:

- **A definition file** — `agents/<name>.md` — role, when to dispatch, system prompt
- **A state folder** — `agents/<name>/` — 4 files maintaining operational memory:
